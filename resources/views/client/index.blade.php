    ...
    <div class="swiper rounded-md !w-full !h-full  !bg-transparent">
        ...
        <div class="swiper-wrapper !w-full !h-full">
            ...
            <div class="swiper-slide !w-full !h-full">
                <img src="{{ asset('uploads/sliders/' . $slider->image) }}" alt="" class="w-full h-full">
                @if ($slider->text != '')
...
<section class=" ">
    <div class="flex flex-wrap justify-around mt-16 max-w-[1400px] mx-auto px-3 sm:px-6 md:px-8">
        ...
        @endif
        @if ($homeCard->title3 != '')
        <div class="hidden lg:block flex-col justify-between border rounded-md max-w-[290px] mt-5 p-4 bg-white">
...
@endif
<!-- main -->
<div class="flex  items-center gap-4 sm:gap-6 flex-col md:flex-row relative h-auto mt-16 max-w-[1400px] mx-auto px-3 sm:px-6 md:px-8">
    @if ($specialProduct)
    <div class="w-full md:w-[50%] mt-4" style="align-item: center; align-self: stretch;">
        <div class="product border-red-600 border-solid !border-2 p-10  bg-w-lws" style="height: 100%">
            @php
            ...
            <div class="flex w-full h-full !justify-center relative group cursor-pointer overflow-clip">
                ...
                <div style="height: 40px" class="px-3 bg-gradient-to-r from-pink-400 to-pink-600  text-white flex-center absolute top-1 left-1 rounded-full">
                    <span class="text-[14px]">{{ $discount }}&#2547; Discount</span>
                </div>
                @endif
                <a href="{{ route('product', $specialProduct->product->slug) }}">
                    <img src="{{ $imageName ? asset('uploads/products/' . $imageName) : asset('client/images/650.png') }}" alt="{{ $specialProduct->product->title }}" class="w-full rounded-lg h-full" />
                </a>
            </div>
            <div class="mt-2 w-full group relative px-4">
                <a href="{{ route('product', $specialProduct->product->slug) }}" class="link pb-1 text-center block font-extrabold" style="font-size: 21px" id="sm25px">{{ $specialProduct->product->title }}</a>
                <p class="flex justify-center items-center" style="margin-bottom: 8px ">
                    ...
                    @for ($i = 1; $i <= $maxStars; $i++) @if ($i <=$filledStars) <i class="fa-solid fa-star text-[15px] text-orange-500"></i>
                        @else
                        <i class="fa-solid fa-star text-[15px] text-gray-500"></i>
                        @endif
                        @endfor
                        <b class="  text-[15px]" style="padding-left: 6px !important">{{ number_format($totalRating, 1) }}</b>
                </p>
                <div class="flex items-center justify-end gap-2">
                    <div class="px-[5px] hidden lg:block mr-16 py-1 bg-white  border rounded-[20px]  transition ease-in duration-150">
                        <button class="mx-4" onclick="addToCart({{ $specialProduct->product->id }})">
                            <i class="fa-solid fa-cart-shopping icons text-slate-600 hover:text-gray-500 transition"></i>
                        </button>
                    ...
                    <span class="text-[20px] font-extrabold bg-gradient-to-r from-rose-700 to-pink-600 text-transparent bg-clip-text">&#2547;{{ number_format($specialProduct->product->price, 2) }}</span>
                </div>
                <div class="px-[5px] block mx-auto lg:hidden w-[200px] py-1 bg-white  border rounded-[20px]  transition ease-in duration-150">
                    <button class="mx-4">
                        <i class="fa-solid fa-cart-shopping icons text-slate-600 hover:text-gray-500 transition"></i>
                    </button>
                    <button class="mx-4">
    ...
    <a href="{{ route('products') }}" class="absolute -top-6 right-5 cursor-pointer border-b-2 border-spacing-1 text-xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-violet-500">
        SPECIAL OFFER</a>
    @if ($latestProduct)
    <div class="w-full md:w-[50%] flex flex-wrap mt-4 gap-y-2 " style="align-items: stretch;">
        @foreach ($latestProduct as $product)
        <div class="HomeSpecialProduct p-[1px] product bg-white   mx-auto border !rounded-none !shadow-none lg:h-[360px] justify-between overflow-hidden">
            <div class="flex justify-center items-center w-full relative group cursor-pointer overflow-clip">
                ...
                <div class="px-2 py-1 bg-red-500 text-white rounded-full flex-center absolute top-1 left-1">
            ...
            <div class="mt-2 w-full p-2">
                <a href="{{ route('product', $product->slug) }}" class="link !text-[15px]">{{ $title }}</a>
                ...
                <div class="flex flex-col sm:flex-row items-center gap-2 justify-center">
                    ...
                    @endif
                    <span class="text-[18px] font-bold text-red-500">&#2547;{{ number_format($product->price, 2) }}</span>
                </div>
            </div>
...
<div class="my-40 w-[80%] mx-auto">

    <div class="products" style="align-items: unset">

        @if ($products->isNotEmpty())
        @foreach ($products as $product)
        <div class="product border-yellow-400 border-solid !border-2" style="justify-content: space-between !important">
            <div class="flex justify-center items-start w-full relative group cursor-pointer overflow-clip">
                ...
                <a href="{{ route('product', $product->slug) }}"><img src="{{ $imageName ? asset('uploads/products/' . $imageName) : asset('client/images/650.png') }}" alt="{{ $product->title }}" class="w-full h-full rounded-lg mx-auto" /></a>
                ...
                <div class="px-[8px] bg-white sm:hidden absolute bottom-1 border rounded-[20px] group-hover:flex transition ease-in duration-150">
                    <i class=""></i>
                    <button onclick="addToCart({{ $product->id }})">
                        <i class="fa-solid fa-cart-shopping icons text-slate-600 hover:text-gray-500 mr-1 transition"></i>
                    </button>
                    <button onclick="addToWishlist({{ $product->id }})">
                        <i class="fa-regular fa-heart icons text-slate-600 hover:text-gray-500 mx-1 transition"></i>
                    </button>
                    <button>
                        <i class="fa-solid fa-magnifying-glass icons text-slate-600 hover:text-gray-500 ml-1 transition"></i>
                    </button>
                </div>
            </div>
            <div class="mt-2 w-full text-center group relative">
                ...
                <a href="{{ route('product', $product->slug) }}" class="link !tex-[12px] sm:!text-[15px]">{{ $product->title }}</a>
                <p class="flex justify-center items-center" style="margin-top: -9px ">
                    ...
                    @for ($i = 1; $i <= $maxStars; $i++) @if ($i <=$filledStars) <i class="fa-solid fa-star text-[13px] text-orange-500"></i>
                        @else
                        <i class="fa-solid fa-star text-[13px] text-gray-500"></i>
                        @endif
                        @endfor
                        <b class="  text-[13px]" style="padding-left: 6px !important">{{ number_format($totalRating, 1) }}</b>
                </p>
...
<div class="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-3 sm:px-6 md:px-8 ">
    ...
    <div class="w-full md:w-full h-[180px] md:h-[200px] lg:h-[190px] xs:h-[300px] xl:h-[250px] rounded-md mx-auto mb-14 lg:mb-0 ">
        <iframe class="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" title="YouTube video player" allowfullscreen frameborder="0" src="{{ $feedback->feedback2 }}"></iframe>
        <div class="w-full">
            <h2 class="text-[22px] font-bold text-center text-blue-dark mt-3 ">{{ $feedback->shopper1 }}</h2>
            <h4 class="text-[18px] text-center">Shopper at Anybringr</h4>
        </div>
    ...
    <div class="w-full md:w-full h-[180px] md:h-[200px] lg:h-[190px] xs:h-[300px] xl:h-[250px] rounded-md mx-auto mb-14 lg:mb-0 ">
        <iframe class="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" title="YouTube video player" allowfullscreen frameborder="0" src="{{ $feedback->feedback3 }}"></iframe>
        <div class="w-full">
            <h2 class="text-[22px] font-bold text-center text-blue-dark mt-3 ">{{ $feedback->shopper1 }}</h2>
            <h4 class="text-[18px] text-center">Shopper at Anybringr</h4>
        </div>
...
@if ($faqs->isNotEmpty())
<div class=" flex justify-center items-start  max-w-[1400px] mx-auto px-3 sm:px-6 md:px-8" style="margin-top: 120px">
        ...
        <ul class="flex flex-col bg-white ">
            ...
            <li class="bg-white my-3 accordion-item border">
                <h2 class="flex flex-row justify-between items-center font-semibold p-3 cursor-pointer">
                    <span>{{ $faq->title }}</span>
                    <svg class="fill-current text-blue-dark h-6 w-6 transform transition-transform duration-500 rotate-icon" viewBox="0 0 20 20">
                        <path d="M13.962,8.885l-3.736,3.739c-0.086,0.086-0.201,0.13-0.314,0.13S9.686,12.71,9.6,12.624l-3.562-3.56C5.863,8.892,5.863,8.611,6.036,8.438c0.175-0.173,0.454-0.173,0.626,0l3.25,3.247l3.426-3.424c0.173-0.172,0.451-0.172,0.624,0C14.137,8.434,14.137,8.712,13.962,8.885 M18.406,10c0,4.644-3.763,8.406-8.406,8.406S1.594,14.644,1.594,10S5.356,1.594,10,1.594S18.406,5.356,18.406,10 M17.521,10c0-4.148-3.373-7.521-7.521-7.521c-4.148,0-7.521,3.374-7.521,7.521c0,4.147,3.374,7.521,7.521,7.521C14.148,17.521,17.521,14.147,17.521,10">
                        </path>
                    </svg>
                </h2>
                <div class="border-l-2 border-gray-700 overflow-hidden max-h-0 duration-500 transition-all tab-content " style="height: 200px">
...
<div class="flex-center flex-col relative mt-16 w-[96%] md:max-w-[1400px] mx-auto px-3 sm:px-6 md:px-8 bg-cover" style="background-image: url('{{ $bannerImage }}')">
    ...
    <a href="{{ $anyBringrSettings->banner_link }}" class="z-10">
        <button class="bg-blue-dark text-white py-3 px-5 border border-white rounded-sm hover:bg-transparent mt-10 mb-20 md:mb-40 z-10">{{ $anyBringrSettings->banner_btn }}</button>
    </a>
...
<div class="w-full md:w-full h-[180px] md:h-[200px] lg:h-[190px] xs:h-[300px] xl:h-[250px] rounded-md mx-auto mb-14 lg:mb-0 ">
    <iframe class="w-full h-full " src="{{ $feedback->feedback3 }}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" loading: 'lazy' allowfullscreen>
    </iframe>
    <div class="w-full">
